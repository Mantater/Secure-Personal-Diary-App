...
def checkYearType(year):
    if (year % 4 == 0 and 
        ...
        print("(Calendar year = Leap year)")

    else:
        print("(Calendar type = Common year)")
...
def date():
    ...
    while dateValid == False:
        try:
            yyyy = int(input("\nYear (1871-now): "))
            # Telling user type of year
            checkYearType(yyyy)
            mm = int(input("Month (1-12): "))
            dd = int(input("Day (1-31): "))
            # Checking validity of date
            datetime.datetime(year=1871, month=1, day=1) < datetime.datetime(year=yyyy,month=mm,day=dd) <= datetime.datetime.today()
            dateValid = True
        except ValueError:
            print("[Invalid date!]")

    userDate = str(dd), str(mm), str(yyyy)
    printDate = '/'.join(userDate)
    userDate = ' '.join(userDate)
    print("\nDate: " + printDate)
    notes.append(printDate)
    week(userDate)
...
def week(userDate):
    # Checks weekday with user's date input
    weekday = datetime.datetime.strptime(userDate, '%d %m %Y').weekday()
    print("Weekday: " + calendar.day_name[weekday])
    notes.append(calendar.day_name[weekday])
#=====================================================================
# List of emotions
emotionList = ["Joy", 
               "Love", 
